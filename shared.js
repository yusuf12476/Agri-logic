// ═══════════════════════════════════════════════════
//  AGRILOGIC SHARED.JS v2.1 — FLAT STRUCTURE
//  All files in root directory (no modules/ folder)
// ═══════════════════════════════════════════════════
...
const LANGS = {en:'EN', sw:'SW', ki:'GĨ', lg:'LG'};
const sw = () => currentLang !== 'en';
const langNote = () => sw() ? 'Respond in Swahili. Keep scientific/disease/product names in English.' : 'Respond in English.';
...
function buildLangToggle() {
  ...
  el.innerHTML = Object.entries(LANGS).map(([code, label]) =>
    `<button class="lang-btn ${code === currentLang ? 'active' : ''}" onclick="setLang('${code}')">${label}</button>`
  ).join('');
}
...
// ── NAVIGATION PAGES ─────────────────────────────────
// ALL pages are in the ROOT — same folder as index.html
const PAGES = [
  ...
  {id:'weather',    label:'Weather Advisor',   icon:'🌤️', href:'weather.html',    section:'diagnose'},
  ...
  {id:'weight',     label:'Weight Tracker',    icon:'⚖️', href:'weight.html',     section:'livestock'},
  ...
  {id:'insurance',  label:'Insurance Calc',    icon:'🛡️', href:'insurance.html',  section:'finance'},
  {id:'subsidy',    label:'Subsidy Finder',    icon:'🏛️', href:'subsidy.html',    section:'finance'},
  ...
  {id:'land',       label:'Land Manager',      icon:'🗺️', href:'land.html',       section:'operations'},
...
function buildSidebar(activePage) {
  ...
  // Farm profile card
  const farmItems = [];
  if (profile.name)     farmItems.push(`🏡 <span>${escHtml(profile.name)}</span>`);
  if (profile.location) farmItems.push(`📍 <span>${escHtml(profile.location)}</span>`);
  if (profile.animal)   farmItems.push(`🐾 <span>${escHtml(profile.animal)}</span>`);
  if (profile.crops)    farmItems.push(`🌾 <span>${escHtml(profile.crops)}</span>`);
  if (profile.size)     farmItems.push(`📐 <span>${escHtml(profile.size)}</span>`);
  ...
  <div class="sb-farm">
    ...
    <div id="sbFarmContent">
      ${farmItems.length
        ? farmItems.map(i => `<div class="sb-farm-item">${i}</div>`).join('')
        : '<div class="sb-farm-item">No farm info yet</div>'}
    </div>
...
function openFarmModal() {
  const profile = getProfile();
  const existing = document.getElementById('farmModal');
  if (existing) { existing.classList.add('open'); populateFarmModal(profile); return; }
  ...
  modal.innerHTML = `
    <div class="modal">
      ...
      <p class="modal-sub">Helps AI give you personalised advice throughout the app.</p>
  ...
  populateFarmModal(profile);
}

function populateFarmModal(profile) {
  ['name','location','crops','size'].forEach(k => {
    const el = document.getElementById('fm-' + k);
    if (el) el.value = profile[k] || '';
  });
  const an = document.getElementById('fm-animal');
  if (an) an.value = profile.animal || '';
  const ex = document.getElementById('fm-exp');
  if (ex) ex.value = profile.exp || '';
}
...
// ── SHARED INIT ───────────────────────────────────────
// Call this at the bottom of every page's <script>
function initShared(activePage) {
  window.AL_PAGE = activePage;
  buildTopbar();
  buildSidebar(activePage);
  setupPWA();
  // Pre-fill today's date on all empty date inputs
